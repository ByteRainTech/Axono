...
// 类型分派的加法
AXONO_FORCE_INLINE Status DispatchAdd(const Tensor &a, const Tensor &b,
                                      Tensor &result) {
  ...
  case DataType::FLOAT32:
    AddKernel(a.data<float>(), b.data<float>(), result.data<float>(),
              num_elements);
    break;
...
// 类型分派的标量加法
AXONO_FORCE_INLINE Status DispatchAddScalar(const Tensor &a, void *scalar,
                                            size_t scalar_size,
                                            Tensor &result) {
  auto num_elements = a.num_elements();
