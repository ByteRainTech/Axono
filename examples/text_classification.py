...
def main():
    ...
    class TextClassifier(axono.nn.Module):
        def __init__(
            ...
            self.embedding = axono.nn.Embedding(vocab_size, d_model, device=device)
            ...
            self.classifier = axono.nn.Linear(d_model, num_classes, device=device)
    ...
    model = TextClassifier(
        vocab_size=len(vocab), num_classes=len(categories), device="cuda:0"
    )

    # 4. 创建优化器和训练器
    optimizer = axono.train.Adam(model.parameters(), lr=0.001)

    trainer = axono.train.Trainer(
        model=model, optimizer=optimizer, loss_fn="cross_entropy", device="cuda:0"
    )
    ...
    # 7. 训练模型
    trainer.fit(
        train_loader=data_loader, epochs=10, callbacks={"on_epoch_end": viz.update}
    )
    ...
    test_tensor = axono.core.Tensor.from_numpy(test_tokens[None, :]).to("cuda:0")
