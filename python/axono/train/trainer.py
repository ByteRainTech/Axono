...
class Trainer:
    ...
    def fit(
        ...
        for epoch in range(epochs):
            ...
            for key in train_metrics[0].keys():
                train_avg_metrics[key] = sum(
                    m[key] for m in train_metrics
                ) / len(train_metrics)
            ...
            if valid_loader is not None:
                ...
                for key in valid_metrics[0].keys():
                    valid_avg_metrics[key] = sum(
                        m[key] for m in valid_metrics
                    ) / len(valid_metrics)
            ...
            if valid_loader is not None:
                metrics_str += ", " + ", ".join(
                    f"valid_{k}: {v:.4f}" 
                    for k, v in valid_avg_metrics.items()
                )
            print(
    ...
    def evaluate(self, test_loader: Any) -> Dict[str, float]:
        ...
        for key in test_metrics[0].keys():
            test_avg_metrics[key] = sum(
                m[key] for m in test_metrics
            ) / len(test_metrics)
