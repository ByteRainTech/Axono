        ...
        def _add_module(module: Module, name: str = ""):
            for child_name, child in module.named_children():
                ...
                self.graph[child_full_name] = {
                    ...
                    'params': {
                        name: tensor.shape
                        for name, tensor in child.named_parameters()
                    },
                    'children': []
                }
...
class TrainingVisualizer:
    ...
    def plot_metrics(self, figsize: Tuple[int, int] = (12, 4)) -> None:
        ...
        # Plot loss
        ax1.plot(epochs, self.history['train']['loss'], 
                'b-', label='Training')
        if self.history['valid']['loss']:
            ax1.plot(epochs, self.history['valid']['loss'], 
                    'r-', label='Validation')
        ax1.set_title('Loss')
        ...
        # Plot accuracy
        ax2.plot(epochs, self.history['train']['accuracy'], 
                'b-', label='Training')
