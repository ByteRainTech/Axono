...
inline void pack_a_block(const float* a, size_t lda,
                         size_t i0, size_t p0,
                         size_t mr, size_t kc)
{
...
inline void pack_b_block(const float* b, size_t ldb,
                         size_t p0, size_t j0,
                         size_t kc, size_t nr)
{
...
extern "C"
void sgemm_kernel_16x6(const float* A, const float* B, float* C,
                       ...
                       float alpha, float beta)
{
    ...
    c8 = c9 = c10 = c11 = c12 = c13 = c14 = c15 = _mm256_setzero_ps();
    ...
    for (std::size_t p = 0; p < k; ++p) {
        ...
        COMPUTE_ROW(0)   COMPUTE_ROW(1)   COMPUTE_ROW(2)   COMPUTE_ROW(3)
        COMPUTE_ROW(4)   COMPUTE_ROW(5)   COMPUTE_ROW(6)   COMPUTE_ROW(7)
        COMPUTE_ROW(8)   COMPUTE_ROW(9)   COMPUTE_ROW(10)  COMPUTE_ROW(11)
        COMPUTE_ROW(12)  COMPUTE_ROW(13)  COMPUTE_ROW(14)  COMPUTE_ROW(15)
        #undef COMPUTE_ROW
...
// 基础优化版本 - 修复内存访问
template<typename T>
void MatMulBasicOptimized(const T *a, const T *b, T *result,
                ...
                for (size_t pp = p; pp < p_end; pp += kc_block) {
                    ...
                    for (; jj + NR <= j_end; jj += NR) {
                        if constexpr (std::is_same_v<T, float>) {
                            pack_b_block(reinterpret_cast<const float*>(b), ldb, pp, jj, pc_cnt, NR);
                        }
                        ...
                        for (; ii + MR <= i_end; ii += MR) {
                            if constexpr (std::is_same_v<T, float>) {
                                pack_a_block(reinterpret_cast<const float*>(a), lda, ii, pp, MR, pc_cnt);
                            }
                            ...
                            if constexpr (std::is_same_v<T, float>) {
                                ...
                                for (size_t mi = 0; mi < MR; ++mi) {
                                    ...
                                    for (size_t ni = 0; ni < NR; ++ni) {
                                        ...
                                        reinterpret_cast<float*>(result)[(ii + mi) * n + jj + ni] += sum;
                                    }
                                }
                            } else {
                                // 通用版本
...
// 修复的多线程版本
template<typename T>
void MatMulParallel(const T *a, const T *b, T *result, size_t m, size_t n, size_t k) {
...
// 优化的多线程版本 - 更好的负载均衡
template<typename T>
void MatMulParallelOptimized(const T *a, const T *b, T *result, size_t m, size_t n, size_t k) {
    // 初始化结果为0
    std::fill(result, result + m * n, T(0));
...
// 主内核函数 - 根据情况选择最优实现
template<typename T>
void MatMulOptimizedKernel(const T *a, const T *b, T *result,
                          size_t m, size_t n, size_t k) {
#if AXONO_USE_X86_INTRINSICS
    if constexpr (std::is_same_v<T, float>) {
